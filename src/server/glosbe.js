...
const glosbeWords = async (language, word) => {
  return new Promise(resolve => {
    const url = encodeURI(`https://glosbe.com/${language}/${defaultLanguage}/${word.toLowerCase()}`)
...
const glosbeTranslate = async (language, text) => {
  return new Promise(resolve => {
    const url = encodeURI(`https://translate.glosbe.com/${language}-${defaultLanguage}/${text.trim()}`)
    axios(url).then(response => {
      const $ = cheerio.load(response.data)
