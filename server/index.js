...
const database = require("./user.js")
...
app.post('/book', upload.single("file"), async (req, res) => {
    try {
        ...
        const lang = langDetect.detect(req.file.originalname)[0][0]

        const title = req.file.originalname.substring(0, req.file.originalname.lastIndexOf('.'))
        const book = await database.save(title, lang)
...
app.get('/books', async (req, res) => {
    if (req.query.book) {
        ...
        return res.status(200).json(await database.get())
})
